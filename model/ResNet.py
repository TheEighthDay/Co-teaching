...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
                 ...
                 norm_layer=None, fc_hidden_dim=None):
        ...
        if self.fc_hidden_dim is None:
            ...
            self.fc1 = nn.Linear(512 * block.expansion, self.fc_hidden_dim)
            self.fc2 = nn.Linear(self.fc_hidden_dim, num_classes)
            self.inplanes = fc_hidden_dim
...
def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    ...
    if pretrained:
        ...
        model.load_state_dict({k: v for k, v in state_dict.items()
                               if k in model_dict
                               and v.size() == model_dict[k].size()}, strict=False)
    return model
