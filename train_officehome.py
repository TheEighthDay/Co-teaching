...
parser.add_argument('--model_name', type=str, default="KD_SRDC",
                    help='model_name')
parser.add_argument('--base_model_name', type=str, default="RN",
                    help='model_name')
parser.add_argument('--da_model_name', type=str, default="SRDC",
                    help='model_name')

parser.add_argument('--root_dir', type=str, default="/datastore/users/kaibin.tian/DE_experiment/OfficeHome/",
                    help='the path to load the data')
parser.add_argument('--base_model_checkpoint', type=str, default="/datastore/users/kaibin.tian/DE_experiment/base_new/RN/officehome/[C].pth",
                    help='base_model_checkpoint ')
parser.add_argument('--da_model_checkpoint', type=str, default="/datastore/users/kaibin.tian/DE_experiment/da_new/log/SRDC/officehome/[C2A].pth",
...
parser.add_argument('--savedir', type=str, default="murual_epochbeat",
                    help='save dir')
...
def train(epoch,logger,source_train_loader, target_train_loader, model,base_model,da_model, optimizer):
    ...
    for batch in range(len_source_train_loader):
        ...
        elif args.da_model_name=="CDAN":
            with torch.no_grad():
                target_soft_label = da_model(data_target)
                target_soft_label = target_soft_label.detach()
                if args.mutual is True:
                    targetmodel_source_soft_label=da_model(data_source)
                    targetmodel_source_soft_label=targetmodel_source_soft_label.detach()
        elif args.da_model_name=="SRDC":
            with torch.no_grad():
                _,_,target_soft_label = da_model(data_target)
                target_soft_label = target_soft_label.detach()
                if args.mutual is True:
                    _,_,targetmodel_source_soft_label=da_model(data_source)
                    targetmodel_source_soft_label=targetmodel_source_soft_label.detach()
        else:
            pass
        ...
        if (args.mixup is True) and (args.mutual is False):
            ...
            mutual_rate =np.random.beta(alpha,beta)
            #mutual_rate = np.random.beta(10,1)
            mix_data,mix_soft_label=utils.mixup_data(data_source,data_target,source_soft_label.softmax(dim=1),target_soft_label.softmax(dim=1))
            mix_output= model(mix_data)
            mix_loss = utils.cross_entropy_loss(mix_output,mix_soft_label)
            sourcemodel_target_loss = utils.cross_entropy_loss(target_output, sourcemodel_target_soft_label.softmax(dim=1), temperature=args.temperature)
...
if __name__ == '__main__':
    ...
    if args.mutual is True:
        ...
        print("dont't start mutual")
    
    if args.mixup is True:
        print("start mixup")
    else:
        print("dont't start mixup")
    ...
    if args.da_model_name=='DANN':
        ...
        da_model = eval(args.da_model_name).CDAN(num_classes=args.num_class,fc_hidden_dim=256,randomized=True)
        pretrained_dict=torch.load(args.da_model_checkpoint)
        new_state_dict = OrderedDict()
        for i,(k, v) in enumerate(pretrained_dict["state_dict"].items()):
            ...
            new_state_dict[k]=v
        da_model.load_state_dict(new_state_dict)
        da_model=da_model.net_G 
        da_model=nn.DataParallel(da_model, device_ids=args.gpus).cuda()
    elif args.da_model_name=="SRDC":
        da_model = eval(args.da_model_name).SRDC(num_classeses=args.num_class,num_neurons=128)
        pretrained_dict=torch.load(args.da_model_checkpoint)
